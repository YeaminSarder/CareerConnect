...
export const getRecommendations = async (req, res) => {
	try {
		...
		const mySkills = me.profile.getSkillsArray()
		const myInterests = me.profile.getInterestsArray()
		...
		const recommendations = others
			.map((user) => {
				...
				const theirSkills = p.getSkillsArray()
				const theirInterests = p.getInterestsArray()
